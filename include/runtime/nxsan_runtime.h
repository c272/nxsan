...
extern "C" __attribute__((always_inline)) void __nxsan_init(void* hBase, size_t hSize);

// Cleans up the nxsan shadow memory.
// If any memory regions are still marked as allocated, an error is generated.
extern "C" __attribute__((always_inline)) void __nxsan_terminate();
...
extern "C" __attribute__((always_inline)) void* __nxsan_malloc(size_t size);
...
extern "C" __attribute__((always_inline)) void __nxsan_free(void* ptr);

...
llvm::Value* AccessInstrumenter::GetPointerOperand(llvm::Instruction& instr) {
    assert((llvm::isa<llvm::LoadInst>(instr) || llvm::isa<llvm::StoreInst>(instr)) &&
...
std::optional<InstrumentMode> AccessInstrumenter::GetInstrumentMode(llvm::Instruction& instr) {
...
InstrumentSize AccessInstrumenter::GetInstrumentSize(llvm::Instruction& instr) {
    ...
    switch (storeSize) {
        case 8: return InstrumentSize::A8;
        case 16: return InstrumentSize::A16;
        case 32: return InstrumentSize::A32;
        case 64: return InstrumentSize::A64;
        default:
            // unk
...
llvm::FunctionCallee AccessInstrumenter::GetInstrument(InstrumentMode mode, InstrumentSize size) {
...
void AccessInstrumenter::DeclareInstruments(llvm::LLVMContext& ctx) {
    ...
    llvm::FunctionType* instrFuncTy = llvm::FunctionType::get(llvm::Type::getVoidTy(ctx),
                                                              llvm::ArrayRef<llvm::Type*>(instrFuncArgs,1),
                                                              false);
    m_loadCallees[InstrumentSize::A8] = m_mod->getOrInsertFunction("__nxsan_report_load8", instrFuncTy);
    m_loadCallees[InstrumentSize::A16] = m_mod->getOrInsertFunction("__nxsan_report_load16", instrFuncTy);
    m_loadCallees[InstrumentSize::A32] = m_mod->getOrInsertFunction("__nxsan_report_load32", instrFuncTy);
    m_loadCallees[InstrumentSize::A64] = m_mod->getOrInsertFunction("__nxsan_report_load64", instrFuncTy);

    m_storeCallees[InstrumentSize::A8] = m_mod->getOrInsertFunction("__nxsan_report_store8", instrFuncTy);
    m_storeCallees[InstrumentSize::A16] = m_mod->getOrInsertFunction("__nxsan_report_store16", instrFuncTy);
    m_storeCallees[InstrumentSize::A32] = m_mod->getOrInsertFunction("__nxsan_report_store32", instrFuncTy);
    m_storeCallees[InstrumentSize::A64] = m_mod->getOrInsertFunction("__nxsan_report_store64", instrFuncTy);
}
